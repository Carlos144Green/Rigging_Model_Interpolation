...
export class boneChange{
  rot: Quat;
  bID: number;
  constructor(theRot: Quat, theID: number){
...
export class GUI implements IGUI {
  ...
  public keyFrames: boneChange[][];
  public curRotQuat: Quat;
  public curKeyFrame: number;
  ...
  public setPose(t: number){
    let CS: number = Math.floor(t);
    let NS: number = Math.floor(t)+1;
    console.log(t,CS,NS);
    let curKF: boneChange[] = this.keyFrames[CS];
    let nextKF: boneChange[] = this.keyFrames[NS];
    let interpT: number = t-CS;
    for(let i = 0; i < nextKF.length;i++){
      let slerpTI: Quat = Quat.slerp(curKF[i].rot, nextKF[i].rot, interpT);
      this.animation.getScene().meshes[0].bones[i].relRot = slerpTI;
    }
    this.animation.getScene().meshes[0].updateAllBones();
  }

  public incrementTime(dT: number) {
    let allRots: Quat[] = [];
  ...
  public drag(mouse: MouseEvent): void {
    ...
    if (this.dragging) {
      ...
      let boneNum: number = this.animation.getScene().meshes[0].boneHighlighted;// I think this gets the bone like i wanted but idk lol
      if (boneNum >=0) {
        this.HasBone = true;
        let bone: Bone = this.animation.getScene().meshes[0].bones[boneNum];
        let newMouseDir = new Vec3([-1 * mouseDir.x, -1 * mouseDir.y, 0]);
        let boneDir: Vec3 = Vec3.difference(bone.endpoint, bone.position); //idk endpoints and pos isnt working here
        let axis: Vec3 = Vec3.cross(boneDir, newMouseDir);
        let myQuat: Quat = Quat.fromAxisAngle(axis, GUI.rotationSpeed);
    ...
    let cam: Vec3 = this.camera.pos(); // camera coords
    //console.log(x,y);
  // Convert the position of the mouse cursor in screen coordinates to a ray in world coordinates
    let xCentered: number = ((2 * x) / this.width) - 1;
    let yCentered: number = -((2 * y) / this.viewPortHeight) + 1;
    let pointer: Vec4 = new Vec4([xCentered, yCentered, -1, 1]);

    let projInv = this.projMatrix().inverse();
    let viewInv = this.viewMatrix().inverse();
    
    let pointerW: Vec4 = viewInv.multiplyVec4(projInv.multiplyVec4(pointer));
    ...
    let highlights: number = this.animation.getScene().meshes[0].boneHighlighted;
    let currBone: Bone = null;
    let boneLen: number = bones.length;
    let shortDist: number = 100000;
    let shortInd: number = -5;
    if(!this.HasBone){
      for (let i=0; i < boneLen; i++) {
        currBone = bones[i];
        //console.log(currBone);
        let gotHit: number = currBone.doesIntersect(cam, rayPoint);
        if(gotHit>0){
  ...
  public dragEnd(mouse: MouseEvent): void {
    ...
    this.prevX = 0;
    this.prevY = 0;
    let curMesh: Mesh = this.animation.getScene().meshes[0];
  ...
  public onKeydown(key: KeyboardEvent): void {
    switch (key.code) {
      case "Digit1": {
        let testMat: Mat3 = new Mat3([0,1,0,-1,0,0,0,0,1]);
        let testQuat: Quat = testMat.toQuat();
        let newTestMat: Mat3 = testQuat.toMat3();
      ...
      case "ArrowLeft": {
        let BH: number = this.animation.getScene().meshes[0].boneHighlighted;
        if(BH>=0){
          let bone: Bone = this.animation.getScene().meshes[0].bones[BH];
          let boneDir: Vec3 = Vec3.difference(bone.endpoint, bone.position);
          let boneDirInit: Vec3 = Vec3.difference(bone.initialEndpoint, bone.initialPosition);
          let axis: Vec3 = boneDirInit.copy();
          axis.normalize();
          let locAxis: Vec3 = bone.rotation.toMat3().multiplyVec3(axis);
          locAxis.normalize();
          let myQuat: Quat = Quat.fromAxisAngle(locAxis, -GUI.rollSpeed);
          bone.justRotate(myQuat.copy());
          this.animation.getScene().meshes[0].updateAllBones();
        }
        else{
      ...
      case "KeyK": {
        if (this.mode === Mode.edit) {
            let myMesh: Mesh =  this.animation.getScene().meshes[0];
            let newArr: boneChange[] = new Array(myMesh.bones.length);
